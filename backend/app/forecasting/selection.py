...
def _normalise(values: list[float]) -> list[float]:
    ...
    return [(v - low) / span if math.isfinite(v) else 1.0 for v in values]
...
def select_model(
    ...
    for index, result in enumerate(usable):
        ...
        composite += penalties.get(result.model, 0.0)
...
def _rationale(winner: ScoredCandidate, scored: list[ScoredCandidate]) -> str:
    ...
    parts = [
        ...
        f"across {winner.result.n_folds} expanding-window folds."
    ]
